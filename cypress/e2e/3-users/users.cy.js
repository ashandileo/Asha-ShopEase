describe("Login Test", () => {
  it("should able to get list of users", () => {
    // Intercept Login API
    cy.intercept("POST", "http://localhost:3000/api/auth/login*").as("login");

    // Intercept Orders API
    cy.intercept("GET", "http://localhost:3000/api/v1/users*").as("getUsers");
  ...
  it("should able to show create user modal", () => {
    // Intercept Login API
    cy.intercept("POST", "http://localhost:3000/api/auth/login*").as("login");
    ...
    // Assert the form with id id="users-form" is visible
    cy.get("#users-form").should("be.visible");
  });
});
