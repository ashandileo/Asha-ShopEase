describe("Login Test", () => {
  it("should able to get list of orders", () => {
    // Intercept Login API
    cy.intercept("POST", "http://localhost:3000/api/auth/login*").as("login");

    // Intercept Orders API
    cy.intercept("GET", "http://localhost:3000/api/v1/orders*").as("getOrders");
  ...
  it("should able to show create order modal", () => {
    // Intercept Login API
    cy.intercept("POST", "http://localhost:3000/api/auth/login").as("login");
    ...
    // Assert the form with id id="create-order-form" is visible
    cy.get("#create-order-form").should("be.visible");
  });
});
